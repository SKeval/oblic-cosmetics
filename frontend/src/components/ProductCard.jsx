...
export default function ProductCard({ product, index = 0 }) {
  ...
  const handleWishlist = (e) => {
    e.preventDefault();
    if (!customer) { navigate(`/account?next=${encodeURIComponent(`/product/${product.id}`)}`); return; }
    ...
    >
      <Link to={`/product/${product.id}`} className="block">
        <div className="relative aspect-square bg-cream-deep overflow-hidden rounded-[2px]">
          {!inStock ? (
            <span className="absolute top-3 left-3 z-10 text-[11px] tracking-[0.08em] px-3 py-1 rounded-full bg-ink text-cream" data-testid="product-badge">
              Out of Stock
            </span>
          ) : product.badges?.[0] && (
          ...
          <button
            ...
            className={`absolute top-3 right-3 z-10 w-8 h-8 rounded-full flex items-center justify-center transition-colors ${wishlisted ? "bg-plum text-cream" : "bg-cream/90 text-ink hover:bg-cream"}`}
          >
          ...
          <img
            ...
            alt={product.name}
            loading="lazy"
            className={`w-full h-full object-cover transition-transform duration-700 group-hover:scale-[1.04] ${!inStock ? "opacity-60" : ""}`}
          />
          {inStock && (
            <button
